...
+ 解决：
 + cors: Access-Control-Allow-Origin: * || <指定origin> ： CORS需要浏览器和服务器同时支持，
 + jsonp:  http://abc.com?callback=handleData ,在handleData 里面处理返回的数据
 + websocket 
 + node代理转发 ,Nginx 反向代理等
...
设置
可以包含`expires`、`max-age`、`domain`、`path`、`secure`。 基本用法：

```javascript
 setCookie('value', 'key', new Date(Date.now() + 3*1000));//设置3s后过期
function setCookie( key, value, expire) {
  window.document.cookie =
    key + '=' + escape(value) + (expire == null ? '' : '; expires=' + expire.toGMTString());
}
```
```javascript
/*\
